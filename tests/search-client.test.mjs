...
test('configured and unconfigured clients always use Pagefind', async () => {
  let loads = 0;
  const loadPagefind = async () => { loads++; return {search: async () => ({results: [result('/docs/page/')]})}; };
  const options = {localOrigin: 'http://127.0.0.1:4321', loadPagefind};
  const local = await createConfiguredSearchClient(options)('query');
  assert.equal(local.total, 1);
  assert.equal(local.engine, 'pagefind');
  assert.equal(loads, 1);
  const production = await createConfiguredSearchClient({...options, localOrigin: origin, production: true})('query');
  assert.equal(production.engine, 'pagefind');
  assert.equal(loads, 2);
  const withEndpoint = await createConfiguredSearchClient({...options, endpoint: 'https://fixture.search.ai.cloudflare.com/search'})('query');
  assert.equal(withEndpoint.engine, 'pagefind');
  assert.equal(loads, 3);
});

test('a configured search client rejects an already aborted signal', async () => {
  let loads = 0;
  const loadPagefind = async () => { loads++; return {search: async () => ({results: [result('/posts/page/')]})}; };
  const client = createConfiguredSearchClient({localOrigin: origin, loadPagefind});
  const controller = new AbortController();
  controller.abort();
  await assert.rejects(client('query', 8, controller.signal), {name: 'AbortError'});
  assert.equal(loads, 0);
});
