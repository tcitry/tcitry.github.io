...
async function fixture(t) {
  ...
  for (const subdirectory of [content, site, 'site/scripts', 'site/dist', 'site/.generated', 'site/node_modules/wrangler/bin']) {
    await mkdir(path.isAbsolute(subdirectory) ? subdirectory : path.join(directory, subdirectory), { recursive: true });
  }
  for (const file of ['release-manifest.mjs', 'verify-release.mjs', 'deploy-verified.mjs', 'theme-package.mjs']) {
...
test('A sealed release deploys exactly its assets without needing the temporary content checkout', async t => {
  ...
  assert.deepEqual(verification, { args: ['--env', 'production'], environment: 'production', deployToken: false, contentToken: false, proToken: false });
  ...
  assert.deepEqual(deployment, { args: ['deploy'], deployToken: true, contentToken: false, proToken: false, repository: false });
