...
try {
  ...
  try {
    ...
    const initialFollowUps = ai.locator('.agent-chat__follow-ups .prompt-suggestion__item');
    assert.ok((await initialFollowUps.count()) >= 2 && (await initialFollowUps.count()) <= 3, 'Completed answers expose follow-up suggestions');
    ...
    assert.equal(await ai.getByRole('region', {name: '继续追问建议', exact: true}).count(), 0, 'Canceled turns never offer success-style follow-ups');
    ...
    await ai.getByRole('region', {name: '继续追问建议', exact: true}).waitFor();
    await ai.locator('.agent-chat__follow-ups .prompt-suggestion__item').first().click();
    ...
    await ai.getByRole('button', {name: '发送问题', exact: true}).click();
    await aiPage.evaluate(() => window.__services.failAi());
    ...
    const sendsBefore = (await state(aiPage)).writes.filter(item => item.name === 'assistant:sendMessage').length;
    await regenerate.click();
    await aiPage.waitForFunction(before => window.__services.getState().writes.filter(item => item.name === 'assistant:sendMessage').length > before, sendsBefore);
    await ai.getByText('正在生成的部分回答。', {exact: true}).waitFor();
