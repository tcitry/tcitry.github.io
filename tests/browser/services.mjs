...
try {
  ...
  for (const width of [1000, 375]) {
    ...
    try {
      ...
      await consultation.getByText('Pro 会员可发起咨询，等待博主回复。', {exact: true}).waitFor();
  ...
  try {
    ...
    await page.getByText('当前账户', {exact: true}).waitFor();
    ...
    await submit.click();
    await page.getByRole('alert').filter({hasText: '当前登录信息缺少用户名，暂时无法发布评论。'}).waitFor();
