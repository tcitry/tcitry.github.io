...
- 日常在 `/Users/yindongliang/code/tcitry.github.io` 的 `main` 分支工作。迁移前的主分支是 `master`；不要将它误记为旧 `main`。临时 worktree 完成合并后再清理，先确认没有未提交工作或需要保留的本地文件。
- 本站使用 Astro + `@tcitry/astro-book`，业务覆写维护在 `src/`，通用主题实现维护在独立 astro-book 仓库。启动和检查入口以 `package.json` 为准；Hugo 的 `layouts/`、`assets/` 和 Makefile 仅用于旧架构备份。
...
- Astro 的 Giscus 配置与页面适用条件位于 `src/layouts/BookLayout.astro`；以下评论规则同样约束 Astro，旧 Hugo 路径只用于对应历史实现。

## Giscus 评论

- Giscus 必须位于页脚前后篇导航之后，并保持 `data-mapping="pathname"`，除非已审计的迁移明确改变它。Astro 通过 `src/layouts/BookLayout.astro` 的 `comments` 插槽接入；旧 Hugo 对应 `layouts/partials/docs/inject/footer.html`。
