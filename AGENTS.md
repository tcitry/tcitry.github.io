...
- 登录认证及个人阅读功能（收藏、阅读进度、私有笔记、账户操作）统一收敛到 AI 圆圈按钮打开的右侧面板，后续在面板内扩展。圆圈入口使用对话气泡类图标，不使用 sparkle / 星芒图标。主导航不再提供“我的阅读”，文章正文上方不再单独展示阅读账户工具；旧 `/me/` 保留为面板的兼容入口。
