...
export function runNpm(args, cwd = root, capture = false) {
  // npm_execpath keeps child installs on the same npm version as npm run setup.
  return process.env.npm_execpath
    ? run(process.execPath, [process.env.npm_execpath, ...args], cwd, capture)
    : run(process.platform === 'win32' ? 'npm.cmd' : 'npm', args, cwd, capture);
}
...
export async function prepareTheme({ localDirectory, verifyLock = false } = {}) {
  ...
  try {
    ...
    if (localDirectory) {
      ...
      await run('git', ['init', '--quiet'], theme);
      // A user-level autocrlf setting must not change the pinned source bytes.
      await run('git', ['config', 'core.autocrlf', 'false'], theme);
      await run('git', ['remote', 'add', 'origin', source.repository], theme);
      await run('git', ['fetch', '--quiet', '--depth=1', 'origin', source.commit], theme);
      await run('git', ['checkout', '--quiet', '--detach', 'FETCH_HEAD'], theme);
      const head = (await run('git', ['rev-parse', 'HEAD'], theme, true)).trim();
      if (head !== source.commit) throw new Error('Fetched theme commit does not match astro-book.source.json.');
      await runNpm(['ci', '--no-audit', '--no-fund'], theme);
    }
    ...
    const output = await runNpm(['pack', '--json', '--pack-destination', scratch], packageDirectory, true);
