...
try {
  ...
  const chat = await fetch('https://yindongliang.com/api/chat', { method: 'GET', signal: AbortSignal.timeout(15_000) });
  assert.equal(chat.status, 405, 'Production chat API is not routed to the Worker');
  ...
  console.log('Production revision and chat API verified; article synchronization may proceed.');
} catch (error) {
