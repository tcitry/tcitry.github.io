...
<section class={`${styles.surface} @container demo-surface not-prose my-6 min-w-0 rounded-2xl border border-border bg-surface text-foreground`} aria-label="Svelte 响应式计数演示" data-demo="svelte-counter">
  ...
  <div class="flex flex-wrap items-center gap-3 border-t border-separator px-5 py-4 sm:px-6">
    <label class="flex items-center gap-2 text-sm">
      步长
      <select bind:value={step} onchange={() => trackDemoStart('svelte-counter', 'step')} disabled={!hydrated} class="rounded-lg border border-border bg-surface px-3 py-2 text-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus disabled:opacity-50">
        <option value={1}>1</option>
        <option value={2}>2</option>
        <option value={5}>5</option>
      </select>
    </label>
    <button type="button" disabled={!hydrated} class="cursor-pointer rounded-lg bg-accent px-4 py-2 text-sm font-medium text-accent-foreground hover:opacity-90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus disabled:opacity-50" onclick={() => {count += step; trackDemoStart('svelte-counter', 'increment');}} data-testid="svelte-increment">增加 {step}</button>
    <button type="button" disabled={!hydrated} class="cursor-pointer rounded-lg border border-border bg-surface px-4 py-2 text-sm text-foreground hover:bg-surface-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus disabled:opacity-50" onclick={() => {count = 0; trackDemoStart('svelte-counter', 'reset');}}>归零</button>
  </div>
