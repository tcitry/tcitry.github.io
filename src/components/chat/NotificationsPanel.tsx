...
interface Props {onOpenConsultation: (threadId: Id<'consultationThreads'>, inbox?: boolean) => void}
...
function notificationLabel(item: {kind: string; target: {kind: string; threaded?: boolean} | null}) {
  if (item.kind === 'comment_reply') return '你的评论收到回复';
  if (item.kind === 'new_comment') return item.target?.kind === 'comment' && item.target.threaded ? '文章有新回复' : '文章有新评论';
  if (item.kind === 'consultation_message') return '咨询有新消息';
  return '博主回复了你的咨询';
}
...
function Notifications({onOpenConsultation}: Props) {
  ...
  return <>
    ...
    {results.length === 0 ? <p className={styles.empty}>还没有消息。评论和咨询的新动态会出现在这里。</p> : <ul className={styles.list}>
      {results.map(item => {
        ...
        return <li className={styles.item} key={item._id} data-unread={item.readAt === null || undefined}>
          ...
          }}>{content}</a> : target?.kind === 'consultation' ? <Button className={styles.link} variant="ghost" isDisabled={Boolean(opening)} aria-busy={opening === item._id} onPress={() => {void open(item._id, item.readAt, () => onOpenConsultation(target.threadId, item.kind === 'consultation_message'));}}>{content}</Button>
