---
title: Labs
description: 前端 demo 的统一入口，收录轨道重力、曼德勃罗集合、Three.js、CSS、HeroUI Pro、HeroUI、Svelte 与 Astro 示例。
...
import ScrollCapsuleNavbar from '../../components/demos/ScrollCapsuleNavbar';
...
export const demoDirectory = [
  ...
  {name: 'Orbit · 轨道重力实验室', description: '放置或抛出天体，观察星系、三体编舞与碰撞。', links: [{label: '打开页面', href: '/demos/2026/orbit/'}, {label: '玩法说明', href: '/docs/Frontend/Tooling/orbit-gravity-lab/'}]},
  {name: '无穷海岸 · 分形探索', description: '全屏探索曼德勃罗集合，切换八个位置与五种配色。', links: [{label: '打开页面', href: '/demos/2026/infinite-coast/'}, {label: '玩法说明', href: '/docs/Frontend/Tooling/infinite-coast/'}]},
  ...
  {name: 'React · 滚动玻璃胶囊导航', description: '滚动内部页面，观察全宽导航通过双阈值收缩为玻璃胶囊。', links: [{label: '本页体验', href: '#react滚动玻璃胶囊导航'}, {label: '技术说明', href: '/docs/Frontend/React/scroll-aware-glass-capsule-navbar/'}]},
...
这个归档案例展示一种已从产品组件中移除、但仍有教学价值的交互：导航在页面顶部保持全宽透明，滚动后收缩为居中的玻璃胶囊。使用内部滚动区域体验，不会改变 Labs 页自身的阅读位置。

<ScrollCapsuleNavbar client:visible />
