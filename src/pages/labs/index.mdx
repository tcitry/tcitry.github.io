---
title: Labs
description: 可以直接操作的前端实验，主要展示 HeroUI Pro 组件，并收录 HeroUI、Svelte 与 Astro 示例。
...
这里收录可以直接操作的前端实验，先从 HeroUI Pro 的选择、评价和消息组件开始。右侧目录可以跳到感兴趣的示例。
...
<div class="my-5 flex flex-wrap items-center gap-3 border-y border-[var(--gray-200)] py-3 text-sm">
  <label htmlFor="labs-reading-theme">试试不同外观</label>
  <select id="labs-reading-theme" data-book-theme-select aria-label="阅读主题" class="rounded-lg border border-[var(--gray-200)] bg-[var(--body-background)] px-3 py-2 text-[var(--body-font-color)]">
...
同一个组件也可以[在独立页面打开](/labs/agent-replay/)。它只回放本地预设数据，不调用 AI 模型或后端服务。
...
- [Three.js：亲手改变一个 3D 场景](/demos/2026/threejs-basics/)：切换 Geometry、Material 与 Light，拖动 Camera，观察线框与 Animation。也可结合 [Three.js 与 Blender 入门](/docs/Frontend/Tooling/threejs-and-blender-guide/) 阅读。
