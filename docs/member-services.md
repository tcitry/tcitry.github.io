...
2026-09-16 入口纠正：已撤回“点击登录直接打开装有 SignIn 表单的独立窗口”方案，恢复原页面官方 Clerk modal，只有点击其中的 Google / GitHub 才启动 OAuth popup。这是恢复既定交互，不代表额外 callback tab 已修复。后续修复必须保留 modal → OAuth popup 的顺序，不得再次整体搬走登录表单。
